...
void HariMain(void) {
    ...
    while(1) {
        ...
        if(fifo8_status(&keyfifo) != 0) {
            ...
            boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 64, 80, 80);
            putfonts8_asc(buf_back, binfo->scrnx, 0, 64, COL8_FFFFFF, s);
            sheet_refresh(sht_back, 0, 64, 72, 80);
        } else if(fifo8_status(&timerfifo2) != 0) {
            i = fifo8_get(&timerfifo2);
            io_sti();
            timer_settime(timer2, 300);
            sprintf(s, "%04d[sec]", timerctl.count/100);
            boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 80, 80, 120);
            putfonts8_asc(buf_back, binfo->scrnx, 0, 80, COL8_FFFFFF, s);
